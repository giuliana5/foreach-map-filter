...
function find(arr, searchValue) {
    const search = arr.filter(function(val, index) {
        if (val === searchValue) {
            return val;
        }
    });
...
function findInObj(arr, key, searchValue) {
    const match = arr.filter(function(val, index) {
...
function removeVowels(str) {
    ...
    let newArray = wordArray.filter(function(val) {
        const vowels = ["a","e","i","o","u"];
        if (vowels.indexOf(val) === -1) {
            return val;
        }
...
function doubleOddNumbers(arr) {
    const filterOdds = arr.filter(function(val) {
        if (!(val % 2 === 0)) {
            return val;
        }
    });

    return filterOdds.map(function(val) {
        return val * 2;
    });
}
